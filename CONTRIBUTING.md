...
Bug reports and feature requests for 4chan-neXT are tracked at **https://github.com/cercos/4chan-next/issues?q=is%3Aopen+sort%3Aupdated-desc**.
...
2. Make sure your browser, 4chan-neXT, and userscript manager (e.g. Greasemonkey, Violentmonkey, or Tampermonkey) are up to date. **Include the versions you're using in bug reports.**
...
7. Test if the bug occurs using the **native extension** with 4chan-neXT disabled. If it does, it's likely a problem with 4chan or your browser rather than with 4chan-neXT.
...
- Clone 4chan-neXT: `git clone https://github.com/cercos/4chan-next.git`
  - If this is taking too long, you can add `--depth 100` to fetch only recent history.
  - Alternatively, if you already have a local 4chan X repo, you can add neXT as a remote:
    `git remote add next https://github.com/cercos/4chan-next.git`
- Open the directory: `cd 4chan-next`
- Fetch needed dependencies with: `npm install`

### Build

- Build for local testing with `npm run testbuild`. Release build options are in the readme.
...
- Fetch needed dependencies with: `npm install`
- Compile the script with: `npm run testbuild`
...
4chan-neXT updates from there automatically.
...
At the moment 4chan-neXT doesn't have its own wiki yet.
